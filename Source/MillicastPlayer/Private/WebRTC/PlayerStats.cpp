	...
	for (const webrtc::RTCStats& Stats : *Report)
	{
		...
		if (Stats.type() == webrtc::RTCInboundRTPStreamStats::kType)
		{
			...
			if (*InboundStat.kind == webrtc::RTCMediaStreamTrackKind::kVideo)
			{
				...
				VideoTotalReceived = ValueOrDefault(InboundStat.bytes_received, 0);
				VideoPacketLoss = ValueOrDefault(InboundStat.packets_lost, -1);
				VideoJitter = ValueOrDefault(InboundStat.jitter, 0) * 1000.;
				VideoDecodeTime = ValueOrDefault(InboundStat.total_decode_time, 0);
				VideoDecodeTimeAverage = 1000. * VideoDecodeTime / (double) *InboundStat.frames_decoded;
				VideoPacketDiscarded = ValueOrDefault(InboundStat.packets_discarded, 0);
				VideoNackCount = ValueOrDefault(InboundStat.nack_count, 0);
				...
				LastVideoReceivedTimestamp = ValueOrDefault(InboundStat.last_packet_received_timestamp, -1);
			...
			else
			{
				...
				AudioTotalReceived = ValueOrDefault(InboundStat.bytes_received, 0);
				AudioPacketLoss = ValueOrDefault(InboundStat.packets_lost, -1);
				AudioJitter = ValueOrDefault(InboundStat.jitter, 0) * 1000.;
				AudioPacketDiscarded = ValueOrDefault(InboundStat.packets_discarded, 0);
				AudioNackCount = ValueOrDefault(InboundStat.nack_count, 0);
				...
				LastAudioReceivedTimestamp = ValueOrDefault(InboundStat.last_packet_received_timestamp, -1);
		...
		else if (Stats.type() == webrtc::RTCMediaStreamTrackStats::kType)
		{
			auto MediaStats = Stats.cast_to<webrtc::RTCMediaStreamTrackStats>();

			if (*MediaStats.kind == webrtc::RTCMediaStreamTrackKind::kVideo)
			{
				Width = ValueOrDefault(MediaStats.frame_width, 0);
				Height = ValueOrDefault(MediaStats.frame_height, 0);
				FramePerSecond = ValueOrDefault(MediaStats.frames_per_second, 0);
				FramesDropped = ValueOrDefault(MediaStats.frames_dropped, 0);
				auto videoJitterDelay = ValueOrDefault(MediaStats.jitter_buffer_delay, -1);
				auto videoJitterEmitted = ValueOrDefault(MediaStats.jitter_buffer_emitted_count, 0);
			...
			{
				ConcealedSamples = ValueOrDefault(MediaStats.concealed_samples, 0);
				SilentConcealedSamples = ValueOrDefault(MediaStats.silent_concealed_samples, 0);
				auto audioJitterDelay = ValueOrDefault(MediaStats.jitter_buffer_delay, -1);
				auto audioJitterEmitted = ValueOrDefault(MediaStats.jitter_buffer_emitted_count, 0);
