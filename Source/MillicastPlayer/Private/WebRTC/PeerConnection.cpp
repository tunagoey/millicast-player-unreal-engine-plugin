...
public:
	explicit FrameTransformer(FWebRTCPeerConnection * InPeerConnection) noexcept : PeerConnection(InPeerConnection) {}
...
void FWebRTCPeerConnection::OnTrack(rtc::scoped_refptr<webrtc::RtpTransceiverInterface> Transceiver)
{
	...
	if (OnVideoTrack && Transceiver->media_type() == cricket::MediaType::MEDIA_TYPE_VIDEO)
	{
		...
		{
			auto Transformer = rtc::make_ref_counted<FrameTransformer>(this);
